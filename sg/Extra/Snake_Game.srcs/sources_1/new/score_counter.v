...
module score_counter(
    ...
    input  [22:0] SCORE_TO_WIN,  // Connect to the output of Mode Selection Module to control the game modes, normal mode or endless mode
    output [13:0] CURR_SCORE,    // Connect to the input of Snake Control Module
    ...
    always@ (posedge CLK) begin
        if (RESET)  // Functionality to reset the score
            ...
            if (REACHED_TRGT)   // Every time the player reaches one target, one mark will be added
    ...
    always@ (posedge CLK) begin
        if (real_count_value >= SCORE_TO_WIN)   // Activate the score-reached signal when the player gets enough scores
            Score_Reached <= 1;
        else
            Score_Reached <= 0;
    end // always
    ...
    // Connect the intermediate registers and wires to the outputs
    assign DIGIT_0 = digit_0;
    assign DIGIT_1 = digit_1;
    assign DIGIT_2 = digit_2;
    assign DIGIT_3 = digit_3;
    assign CURR_SCORE    = real_count_value;
